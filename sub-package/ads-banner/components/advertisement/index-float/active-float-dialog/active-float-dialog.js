...
import { userBehaviorEeventTrack } from '../../../../../../track/track.js'
Component({
  properties: {
  ...
  data: {
    ...
    selectData: {},
    adStatus: 'show', // show hide
    adAnimation: null,
    adAnimationData: null,
    aniTimeout: null,
  },
  methods: {
    getInitData(list) {
      ...
      console.log('浮窗选中的广告', selectItem)
    ...
    createAnimation() {
      const adAnimation = wx.createAnimation({
        duration: 375,
        timingFunction: 'linear',
      })
      this.data.adAnimation = adAnimation
    },
    setAdHide(isImmediate) {
      const adShow = this.data.show && this.data.selectData.contentUrl
      if (!adShow) {
        return
      }
      const adAnimation = this.data.adAnimation
      adAnimation.right(-30).opacity(0.3).step()
      const adAnimationData = adAnimation.export()
      if (isImmediate) {
        this.resetAnimationTimeout()
        this.setData({
          adAnimationData,
        })
        this.data.adStatus = 'hide'
      } else {
        if (this.data.aniTimeout) return
        this.data.aniTimeout = setTimeout(() => {
          this.setData({
            adAnimationData,
          })
          this.data.adStatus = 'hide'
    ...
    setAdShow() {
      ...
      this.resetAnimationTimeout()
      const adAnimation = this.data.adAnimation
      adAnimation.right('26rpx').opacity(1).step()
      this.data.adStatus = 'show'
      this.setData(
        {
    ...
    resetAnimationTimeout() {
      if (this.data.aniTimeout) {
        clearTimeout(this.data.aniTimeout)
        this.data.aniTimeout = null
      }
    },
  },
  /*组件生命周期*/
