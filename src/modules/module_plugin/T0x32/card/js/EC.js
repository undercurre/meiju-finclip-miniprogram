...
export class EC {
  // 设置状态栏信息
  static setStatusBarInfo(deviceInfo, statusBarModel, enabledClick) {
    statusBarModel.enabledClick = false
    statusBarModel.isShowInfo = false
    ...
    if (deviceInfo.isRunning) {
      ...
      if (deviceInfo.waterShortageState === 'waterShortage') {
        statusBarModel.type = 'error'
        statusBarModel.text = '缺水中'
      }
    }
    if (deviceInfo.isError) {
      statusBarModel.type = 'error'
      if (deviceInfo.waterShortageState !== 'waterShortage') {
        statusBarModel.text = '故障中'
      } else {
        statusBarModel.text = '缺水中'
      }
      statusBarModel.enabledClick = false
    ...
    if (enabledClick) {
      statusBarModel.enabledClick = enabledClick === 2
    }
    return statusBarModel
  }

  // 检查能否启动
  static isEnabledToStart(deviceInfo) {
    if (deviceInfo) {
      if (deviceInfo.isError) {
  ...
  static quickDevJson2Local(quickDevJson) {
    ...
    if (quickDevJson.properties && quickDevJson.properties.length > 0) {
      ...
      quickDevJson.properties.forEach((propertyItem) => {
        ...
        // 工作状态处理
        if (propertyItem.settings[0].apiKey === 'workStatus') {
