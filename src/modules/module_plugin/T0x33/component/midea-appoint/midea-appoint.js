import { addZero, getCurrentAppointTime, mm2HHmmText } from '../../card/js/plugin-config'
Component({
  options: {
    multipleSlots: true,
  },
  ...
  methods: {
    ...
    scheduleDataInit() {
      ...
      console.log('nowMin + bindModel.defaultValue * 1 > 60', nowMin + bindModel.defaultValue * 1)
      // let minAppointMinute = (nowMin + bindModel.workTime * 1) % 60 // 最小预约分钟
      let minAppointMinute = (nowMin + bindModel.min * 1) % 60 // 最小预约分钟

      // let minAppointHour = nowHour + parseInt((nowMin + bindModel.workTime * 1) / 60) // 最小预约小时

      let minAppointHour = 0
      if (bindModel.min < 60) {
        if (nowMin + bindModel.min * 1 > 60) {
          minAppointHour = nowHour + parseInt((bindModel.min * 1) / 60) + 1 // 最小预约小时
        } else {
          minAppointHour = nowHour + parseInt((bindModel.min * 1) / 60)
        }
      } else {
        minAppointHour = nowHour + parseInt((bindModel.min * 1) / 60) // 最小预约小时
      }

      console.log('当前分钟：', nowMin)
      console.log('最小预约分钟：', minAppointMinute)
      console.log('最小预约小时：', minAppointHour)
      ...
      for (let i = minAppointHour; i <= maxAppointHour; i++) {
        // 遍历小时
        let minutes = [],
          curHour = 0
        if (i < 24) {
        ...
        if (minuteObj[curHour]) {
          minuteObj[`${curHour}明`] = minutes
        } else {
      ...
      // 设置当前的预约时间
      let currentAppoint = getCurrentAppointTime(bindModel.min * 1)
      // let currentAppoint = getCurrentAppointTime(60)
    ...
    pickOnHour(e) {
      ...
      console.log('minuteObj', minuteObj)
      let minutes = []
      if (curDay === '明天' && minuteObj[`${curHour}明`]) {
        minutes = minuteObj[`${curHour}明`]
      } else {
        minutes = minuteObj[curHour]
      }
    ...
    save() {
      ...
      const now = new Date(),
        nowH = now.getHours(),
        nowM = now.getMinutes()
      let aitH = this.data.curHour,
        aitM = this.data.curMin * 1
      aitH += curDay == '明天' ? 24 : 0
