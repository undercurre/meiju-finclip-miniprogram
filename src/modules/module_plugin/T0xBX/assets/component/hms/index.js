...
Component({
  ...
  observers: {
    'min, max, value': function (min, max, value) {
      logger.info('observers', 'min', min)
      logger.info('observers', 'max', max)
      logger.info('observers', 'value', value)
      this.reset()
    },
  },
  attached() {
    logger.setNamespace('hms')
    this.reset()
  },
  detached() {},
  methods: {
    reset() {
      ...
      logger.info('init valueIndexArray', valueIndexArray)
      this.setData({
        valueIndexArray,
      })
    },
    ...
    bindChange: function (e) {
      ...
      logger.info('valueIndexArray', valueIndexArray)

      this.data.value = this.getHms(valueIndexArray)
      logger.info('value', this.data.value)

      // 2. 分别更新时分秒范围
      // 注意：如果更新options后，valueIndexArray 索引溢出会强行出发bindChange事件
