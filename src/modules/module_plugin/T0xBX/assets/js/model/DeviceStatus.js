...
class DeviceStatus {
  ...
  getTips(validations) {
    ...
    for(let attr of validations) {
      ...
      if(TIPS && TIPS[messageKey] && TIPS[messageKey]['conditions']) {
  ...
  _showCountDown() {
    // 应该是用remainingSeconds 判断倒计时，而非settingSeconds，因为有的设备工作中没上报设定的时间
    // console.log(this.isWorking,this.remainingSeconds,this.isPreheating)
    return this.isWorking && this.remainingSeconds && !this.isPreheating && !this.isProbeMode && !this.isPreheatingTempArrived && !this.isSensing
  }
