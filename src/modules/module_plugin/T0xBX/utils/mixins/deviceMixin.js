...
module.exports = Behavior({
  ...
  renderingQuerySuccessCallback(res) {
    ...
    // console.log('working_currentMode:', currentMode)
    ...
    if(deviceStatus.work_status === "work_finish") {
      this.updateCurrentPageData([
        {
          method: 'hideParamsPopup', 
          params: deviceStatus
        },
      ])
    }     
  ...
  updateCurrentPageData(callbacks) {
    ...
    luaControl(params) {
      ...
      return new Promise((resolve, reject) => {
          ...
          console.log("pluginData:",this.data.pluginData);
          console.log("controlParams:",params);
          console.log("deviceStatus:",this.data.deviceStatus);
          let deviceStatus = this.data.deviceStatus
          if(deviceStatus.isProbeMode && (deviceStatus.work_status == "standby" || deviceStatus.work_status == "save_power")) {
            wx.showToast({
                title: '探针模式，请在设备上操作探针模式',
                icon: 'none',
                duration: 2000
            })
            return
