...
// import {
//   default as crypto
// } from './crypto/bluetooth-crypto.js'
...
const app = getApp();
const crypto = app.globalData.ble.default;
...
export default class BluetoothConn {
  ...
  constructor(object) {
    ...
    retryLinkBlueNum = 3
    wx.onAppShow((result) => {
      this.bluetoothMsgCount = undefined;
      console.log('重置bluetoothMsgCount');
    })
    // wx.onAppHide((res) => {      
    //   this.bluetoothMsgCount = undefined;
    //   console.log('重置bluetoothMsgCount');
  ...
  connction() {
    ...
    // this.refreshCacheOpenBluetoothAdapter();
    console.log('开始建立连接...')
    // try {
        ...
        success: (res) => {
          ...
          that.connTimeout = setTimeout(() => {
            if (!that.connSuccess) {
              ...
              that.startNegotiate(); //重新秘钥协商
            }
          }, 30 * 1000); //20秒未协商好主动断开连接
  ...
  closeBLEConnection(callback) {
    // console.log("小程序主动断开连接", this.deviceId)
      ...
      while (that.msgGroup.messageList.length > 0) {
        ...
        switch (msg.type) {
          ...
          case ProtocolType.t2: //安全层
            console.log('安全层这个地方出来-----', that.rootKey, msg.body);
  ...
  checkMsgCount(msg) {
    ...
    console.log(msg,"安全层消息计数器");
    if (!this.bluetoothMsgCount) {
      console.log("!this.bluetoothMsgCount")
      ...
      let allowMiss = 3; //允许丢包数
      if (bluetoothMsgCount > this.bluetoothMsgCount) {
        if (bluetoothMsgCount - this.bluetoothMsgCount <= allowMiss + 1) {
          this.bluetoothMsgCount = bluetoothMsgCount;
          return true;
        }
      } else if (bluetoothMsgCount < this.bluetoothMsgCount) {
        if (bluetoothMsgCount + 255 - this.bluetoothMsgCount <= allowMiss + 1) {
          this.bluetoothMsgCount = bluetoothMsgCount;
          return true;
        }
      }
      ...
      console.log(`消息计算器校验失败，预期${this.bluetoothMsgCount+1}，收到${bluetoothMsgCount}`)
      this.event.dispatch("fail", `消息计算器校验失败，预期${this.bluetoothMsgCount+1}，收到${bluetoothMsgCount}`)
      return false;
    }
  }
  ...
  handleBizMsgC4(msg) {
    ...
    this.event.dispatch("receiveMessagePlugin", ProtocolBiz.decode(data))
  }
