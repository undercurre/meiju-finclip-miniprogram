          ...
          for (let i = 0; i < res.characteristics.length; i++) {
            ...
            if (item.properties.write) {
              ...
              if (item.uuid.includes('FF81') || item.uuid.includes('FF91')) {
                ...
                // this.writeData(this.data.currentOrder) // 放到notifyBLECharacteristicValueChange的回调中调用
              }
        ...
        requestService
          ...
          .then((resp) => {
            ...
            if (resp.data.data.sessionSecret) {
              getApp().globalData.bleSessionSecret = resp.data.data.sessionSecret.replace(/,/g, '')
              console.log('设置会话秘钥成功', getApp().globalData.bleSessionSecret)
            }
