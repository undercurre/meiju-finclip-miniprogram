...
class Wrapper {
  constructor(isUsingPlugin) {
    ...
    this.globalVarApp = App // 小程序原App对象
    this.globalVarPage = Page // 小程序原Page对象
    if (!isUsingPlugin) {
      App = (app) => this.globalVarApp(this._create(app, this.injectAppMethods, this.extraAppMethods))
      Page = (page) => this.globalVarPage(this._create(page, this.injectPageMethods, this.extraPageMethods))
    }
  }
  ...
  createApp(app) {
    this.globalVarApp(this._create(app, this.injectAppMethods, this.extraAppMethods))
  }

  createPage(page) {
    this.globalVarPage(this._create(page, this.injectPageMethods, this.extraPageMethods))
  }
}
