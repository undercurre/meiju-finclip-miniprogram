...
import { ab2hex, delay } from 'm-utilsdk/index'
...
module.exports = Behavior({
  ...
  methods: {
     ...
     */
    async checkNearby(deviceId, referenceRSSI, downlinkThreshold, checkDistance, ifResolveOnCheck) {
      console.log('@module checkAuthMixin.js\n@method checkNearby\n@desc 开始计算距离\n', {
        deviceId,
        referenceRSSI,
        downlinkThreshold,
      ...
      await delay(1000) // 新增1000毫秒延迟调用，防止wx.startBluetoothDevicesDiscovery调用失败
      return new Promise((resolve) => {
        let RSSIList = []
        openAdapter()
          .then(() => {
