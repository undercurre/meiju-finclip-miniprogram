...
export default class BluetoothConn {
    ...
    connction() {
        ...
        wx.createBLEConnection({
            ...
            success: (res) => {
                ...
                that.connTimeout = setTimeout(async () => {
                    if (!that.connSuccess) {
                        console.log('协商密钥超时')
                        that.event.dispatch("fail", '协商密钥超时')
                        // that.closeBLEConnection();
                        await this.refreshCacheOpenBluetoothAdapter()
                        this.connction();//重新连接协商
                    }
                }, 20 * 1000); //20秒未协商好主动断开连接

                if (app.platform == 'ios') { //errMsg: "setBLEMTU:fail can only be invoked on android"
                    //获取蓝牙所有服务
            ...
            fail: (res) => {
                ...
                //   }
                // })
                if ((res.errCode == 10003 || res.errCode == 10012) && retryLinkBlueNum > 0) {//蓝牙失败或超时重试
                    retryLinkBlueNum--
                    setTimeout(async () => {
                        await this.refreshCacheOpenBluetoothAdapter()
                        this.connction();
    ...
    notify() {
        ...
        wx.notifyBLECharacteristicValueChange({
            ...
            success(res) {
                ...
                if (app.platform == 'ios' || wx.getDeviceInfo().platform == 'ios') { //errMsg: "setBLEMTU:fail can only be invoked on android"
                    //获取蓝牙所有服务
                    //iOS平台上后续对特征值的read、write、notify，由于系统需要获取特征值实例，传入的 serviceId 与 characteristicId 必须由 getBLEDeviceServices 与 getBLEDeviceCharacteristics 中获取到后才能使用。
                    that.startNegotiate();
                } else {
    ...
    async createRootKey() {
        ...
        this.crypto = await require.async(bluetoothCrypto)
        this.rootKey = this.crypto.default.hkdfKeyWithSalt(this.advertisData, 16, [], Common.string2Uint8Array('midea_bleapp'));
    ...
    step1() {
        ...
        if (this.sessionKey) {
            ...
            data = Common.concatUint8Array([data, this.crypto.default.cipherMsg(this.sessionKey, this.advertisData)])
        }
     ...
     * 发送安全层消息
     */
    async sendSecurityLayerMsg(command, data) {
        ...
        if (command == Commands.c4) {
            retry = 0;
            msg = ProtocolConn.encode(ProtocolType.t3, this.crypto.default.cipherMsg(this.sessionKey, smsg));
        } else {
            retry = 2; //协商消息需要连接3次， 即再尝试2次
            msg = ProtocolConn.encode(ProtocolType.t2, this.crypto.default.cipherMsg(this.rootKey, smsg));
        }
    ...
    handleSecurityLayerMsg(key, msg) {
        ...
        msg = this.crypto.default.decipherMsg(key, msg);
    ...
    handleSecurityLayerMsgC2(msg) {
        ...
        this.priKey = this.crypto.default.createPriKey();
        this.pubKey = this.crypto.default.createPubKey(this.priKey);
        this.sessionKey = this.crypto.default.createSecret(this.priKey, this.bluetoothPubKey)
        ...
        console.log("密文", Common.ab2hex(this.crypto.default.cipherMsg(this.sessionKey, this.advertisData)))

        let data = Common.concatUint8Array([this.pubKey, this.crypto.default.cipherMsg(this.sessionKey, this.advertisData)])
    ...
    writePackage(data, offset) {
        ...
        console.log("发送分包", offset, offset + pack.length)
