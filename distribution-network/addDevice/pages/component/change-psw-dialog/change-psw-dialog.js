...
Component({
  ...
  data: {
    // 这里是一些组件内部数据
    wifiShow:imgUrl+imgesList.wifiShow,
    wifiHide:imgUrl+imgesList.wifiHide,
  },
  methods: {
    //自定义事件
    switchPswShow(){
      this.setData({
        ['customDialog.isCanSeePsw']: !this.properties.customDialog.isCanSeePsw
      })
    },
    clickCancel(e) {
      //取消
    ...
    clickConfirm() {
      ...
      this.properties.customDialog.success({
        comfirm: true,
        confirm: true,
        psw:this.properties.customDialog.wifiInfo.PswContent
      })
      this.setData({
        //关闭
        ['customDialog.show']: false,
    ...
    getPsw(e) {
      ...
      this.setData({
        psw: psw,
        'customDialog.wifiInfo.PswContent':psw
      })
    },
  },
  /*组件生命周期*/
