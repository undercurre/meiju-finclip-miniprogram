...
module.exports = Behavior({
  ...
  methods: {
    ...
    createBLEConnection(deviceId) {
      ...
      wx.createBLEConnection({
        ...
        fail: (error) => {
          ...
          if (!this.data.connected && this.data.tryConnectNum > 0) { //重连
    ...
    getBluetoothApplianceInfo() {
      let self = this
      bleNegotiationService.getBluetoothApplianceInfo().then((resp) => { // 获取设备信息指令
    ...
    onBLECharacteristicValueChangeIOS(value, characteristic) {
      ...
      if (this.data.progress == 1) {
        ...
        bleNegotiationService.verifySecretKey(order).then((resp) => {
      ...
      if (this.data.progress == 2) {
        ...
        bleNegotiationService.queryBindCode(sn, modelType, actionType).then((resp) => {
      ...
      if (this.data.progress == 3) {
        ...
        bleNegotiationService.bindCodeResult(order).then((resp) => {
          if (!this.data.isDirectCon) {
            //非直连 不需要获取绑定码的操作了
            console.log('结束确权')
            return
    ...
    isGroup(value) {
      ...
      if (value.includes('aa55') && value.length == this.data.orderLen) { //完整包
        return value
      }
      if (value.includes('aa55') && (value.length > this.data.orderLen) && value.split('aa55').length > 2) { //粘包处理
        let orderArr = value.split('aa55')
        console.log('[粘包]', orderArr)
        return 'aa55' + orderArr[orderArr.length - 2]
      }
      if (value.includes('aa55') && value.length < this.data.orderLen) { //需要组包
        this.data.groudOrder = value
        return false
      }
      if (groudOrder && !value.includes('aa55')) { //子包
        this.data.groudOrder += value
      }
      if (this.data.groudOrder.length == this.data.orderLen) { //组包完成
