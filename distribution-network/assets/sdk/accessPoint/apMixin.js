  ...
  methods: {
    async apSendWifiInfo(tcp, address, port) {
      ...
      tcp.onError(async (error) => {
        ...
        if (this.data.tcpRetryNum > 0) { // todo 判断条件待确定
          setTimeout(() => {
      ...
      if (app.globalData.systemInfo.system.includes("Android") && typeof tcp.bindWifi ===
        'function') { //安卓支持tcp.bindWifi 则先bindWifi
        ...
        wx.getNetworkType({
          success(res) {
            ...
            if (networkType == 'wifi') { //如果当前已经连接wifi,则直接连接
              tcp.connect({
                address: address,
                port: port,
                timeout: 5, //wx 8.0.22 add
              })
            } else { //如果当前还没有连接wifi，通过监听wifi状态变化，转为wifi后再连接
