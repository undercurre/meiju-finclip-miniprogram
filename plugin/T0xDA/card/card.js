...
import statusToLua from '../da_status_to_lua'
import luaToStatus from '../da_lua_to_status'
...
Component({
  ...
  data: {
    ...
    powerImg: {
      on: imageApi.getImagePath.url + '/0xDA/icon_switch_on01@3x.png',
      off: imageApi.getImagePath.url + '/0xDA/icon_switch_off01@3x.png',
    },
    // startImg: {
    //   on: imageApi.getImagePath.url + '/0xDA/icon_start_on01@3x.png',
    //   off: imageApi.getImagePath.url + '/0xDA/icon_start_off01@3x.png',
    // },
    startImg: {
    ...
    // pauseImg: {
    //   on: imageApi.getImagePath.url + '/0xDA/icon_pause_on01@3x.png',
    //   off: imageApi.getImagePath.url + '/0xDA/icon_pause_off01@3x.png',
    // },
    pauseImg: {
    ...
    circleImg: {
      on: imageApi.getImagePath.url + '/0xDA/img_wash_machine_circle01@3x.png',
      off: imageApi.getImagePath.url + '/0xDA/img_wash_machine_circle04@3x.png',
    },
    modeImg: {
    ...
    isAutoInput: false,
    power: {
      // mainImg: imageApi.getImagePath.url + '/0xDA/icon_switch_off01@3x.png',
      mainImg: './meiju_lite_powerOnOff.png',
      desc: '关机',
    },
    circleSrc: imageApi.getImagePath.url + '/0xDA/img_wash_machine_circle04@3x.png',
    machineIcon: imageApi.getImagePath.url + '/0xDA/img_wash-machine@3x.png',
    animationData: {},
    running_status1: {
  ...
  methods: {
    ...
    renderRemainTime() {
      ...
      if (this.data.applianceStatus.running_status == 'end' || this.data.applianceStatus.intelligent_wash == 'on') {
        //向下拖动刷新，预约、洗涤结束或者智能洗（凡帝罗）开启将剩余时间显示为'--'
        remain_time = '--'
      }
      this.setData({
        'applianceStatus.remain_time': remain_time,
    ...
    checkRunningAnimation() {
      if (this.data.applianceStatus.running_status == 'work') {
    ...
    controlPower(p) {
      ...
      let power = p
      let reqId = getReqId()
      this.luaControl({ power: power }, reqId).then(() => {
        // beging 添加字节埋点：电源开关
        let param = {}
        param['page_name'] = '首页'
        param['object'] = '电源开关'
    ...
    powerToggle() {
      ...
      if (
        power == 'off' &&
        (this.data.applianceStatus.running_status === 'work' ||
          this.data.applianceStatus.running_status === 'pause' ||
          this.data.applianceStatus.running_status === 'order')
      ) {
        wx.showModal({
          title: '确认提示',
          content: '是否确认关机？',
      ...
      if (
        ...
        let exStatus =
          this.data.applianceStatus.running_status == 'idle' || this.data.applianceStatus.running_status == 'standby'
            ? '待机'
            : '暂停'
        this.luaControl({ control_status: control_status }, reqId).then(() => {
          // beging 添加字节埋点：启动设备
          let param = {}
          param['page_name'] = '首页'
          param['object'] = '启动程序'
    ...
    luaQuery(manual) {
      ...
      return new Promise((resolve, reject) => {
        ...
        requestService
          ...
          .catch((err) => {
            // console.log('da err: ' + JSON.stringify(err))
