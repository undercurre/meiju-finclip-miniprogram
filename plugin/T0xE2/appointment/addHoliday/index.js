...
Page({
  ...
  data: {
    ...
    loopList:["仅一次", "每天", "法定工作日", "法定节假日", "自定义"], // 
    // 云管家互斥处理
    isCloudOn: false,
  ...
  computed: {
    ...
    holidayTips(){
      if(this.data.defaultLoopIndex==2){
        return '智能跳过法定节假日'
      }else if(this.data.defaultLoopIndex==3){
        return '智能跳过法定工作日'
      }else{
        return ''
      }
  ...
  onLoad({ data }) {
    ...
    if (action == "update") {
      ...
      } else if (task.legalDate && task.legalDate == 2) {
        this.setData({ defaultLoopIndex: 2 });
      } else if (task.legalDate && task.legalDate == 1) {
  ...
  onLoopItemClick({ detail: index }) {
    ...
    if (index == 0) {
      ...
      this.setData({ "task.isRepeat": true });
      this.setData({ "task.legalDate": 2 });
    } else if (index == 3) {
      // 周末
      selectedDays = [6, 0];
      this.setData({ "task.isRepeat": true });
      this.setData({ "task.legalDate": 1 });
    } else {
      // 自定义
      this.setData({ "task.isRepeat": true });
    }
  ...
  saveAppoint() {
    ...
    requestService
      ...
      .then((rs) => {
        ...
        // 埋点
        let params = getModeBuryParams('更新节假日预约', this.data.task, {})
