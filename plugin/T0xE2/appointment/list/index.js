...
Page({
  ...
  computed: {
    ...
    actionParams() {
      ...
      return getModeBuryParams('分段预约开关', lastActionObj, changeObj)
    },
  },
  ...
  toggleAppoint(item) {
    ...
    requestService
      ...
      .then((rs) => {
        ...
        this.queryAppoint(!this.data.isNextDay)
      })
      .catch((e) => {
      ...
      .then(({ data: rs }) => {
        ...
        for (let i = 0; i < result.length; i++) {
          ...
          let appoint = {
            ...
            wkStr: this.parseWeek(result[i].week, result[i].isRepeat),
            week: result[i].week,
  ...
  deleteAppoint({
    ...
    wx.showModal({
      ...
      success: (res) => {
        // 埋点
        let params = getModeBuryParams('删除分段预约', item, {})
