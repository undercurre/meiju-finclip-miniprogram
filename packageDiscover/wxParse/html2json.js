  ...
  HTMLParser(html, {
    start: function (tag, attrs, unary) {
      ...
      if (node.tag === 'img') {
        ...
        var imgUrl = node.attr.src ? node.attr.src : ''
        if (imgUrl[0] == '') {
          imgUrl.splice(0, 1)
        }
      ...
      //临时记录source资源
      if (node.tag === 'source' || node.tag == 'video') {
        if (node.attr.src) {
          results.source = node.attr.src
        }
        if (node.attr['data-third-party-id']) {
          results.videoid = node.attr['data-third-party-id']
        }
      }
      ...
      if (node.tag === 'video' && results.source) {
        ...
        node.attr.videoid = results.videoid
